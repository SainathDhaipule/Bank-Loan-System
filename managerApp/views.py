...
def approved_request(request, id):
    ...
    loan_obj.save()
    months = loan_obj.year * 12  # Convert years to months

    # Retrieve the loanCategory instance based on the category_id field
    loan_category = loanCategory.objects.get(id=loan_obj.category_id)

    monthly_interest_rate = loan_category.interest_rate / 12  # Calculate monthly interest rate
    ...
    if CustomerLoan.objects.filter(customer=approved_customer).exists():
        ...
        total_loan_with_interest = int(loan_obj.amount) * (1 + (monthly_interest_rate / 100) * months)
        CustomerLoan.objects.filter(
            customer=approved_customer).update(total_loan=int(PreviousAmount) + total_loan_with_interest)
        CustomerLoan.objects.filter(
            customer=approved_customer).update(payable_loan=int(PreviousPayable) + total_loan_with_interest)
        ...
        total_loan_with_interest = int(loan_obj.amount) * (1 + (monthly_interest_rate / 100) * months)
        save_loan.total_loan = total_loan_with_interest
        save_loan.payable_loan = total_loan_with_interest
        save_loan.save()
